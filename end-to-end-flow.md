...
5. Click **Admin · ingestion** → drag a PDF or paste a URL of a system-design article → wait for the green toast with the chunk count. *(Backend verifies the Clerk JWT before accepting the upload, and writes your Clerk user id onto `documents.user_id`.)*
6. *(Phase 9)* Scroll down to **Your knowledge base** — the doc you just ingested appears with its title and chunk count. Hit **Delete** on a row to fire a sonner confirmation toast; confirm to remove the row and its chunks (CASCADE). Sign in as a second Clerk user in an incognito window and confirm you see a different (empty or distinct) list.
7. Click **← Back to chat** (or open `/chat`).
8. Type a grounded question (e.g. *"Summarise the key design choices."*) — watch tokens land.
9. Type something outside the corpus (e.g. *"Explain WhatsApp architecture"* when you only uploaded Amazon docs) — watch the Tutor refuse with the literal fallback sentence.
10. Click **Test my knowledge** with the chat input filled — watch a structured MCQ stream in.
11. Reload the page — your conversation is back (resumed from Redis via the `localStorage` session id + the Clerk session cookie).
12. In another shell, `docker exec tutor-redis redis-cli --scan --pattern 'chat:user:user_*'` — you'll see one `chat:user:user_<your-clerk-id>:session:<uuid>:messages` key per conversation. Different Clerk users get separate namespaces.
